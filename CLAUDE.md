...
- **Non-fatal by design**: per-table analysis failures, plan-cache scrape, the LLM step, every health-check step, and every per-database step in the permissions report all catch and continue — one bad check (or one database the login can't see into) must not abort the run. Preserve this.
