...
public static class Narrative
{
    ...
    public static string Build(AnalyzerOptions opt, List<TableInfo> tables)
    {
        ...
        sb.AppendLine($"_Generated {DateTime.Now:yyyy-MM-dd HH:mm}. Rule-based analysis; verify top findings in non-prod before acting._");
        sb.AppendLine();

        // ---- Cross-report data-quality checks ------------------------------------------
        bool allUsageZero = tables.All(t => t.UserSeeks + t.UserScans + t.UserLookups + t.UserUpdates == 0);
        bool cacheShowsActivity = tables.Any(t => t.ReferencingQueries.Count > 0);
        if (allUsageZero && cacheShowsActivity)
        {
            sb.AppendLine("> **Data-quality flag:** every table shows zero index usage, yet the plan cache proves recent " +
    ...
    private static string Verdict(TableInfo t)
    {
        ...
        bool tooSmall = t.PotentialFullRowgroups < 3;
        bool aggEvidence = TopUserQueries(t).Any(q => IsAggregation(q.StatementText));
        bool insertOnly = IsInsertOnly(t);

        if (t.CandidacyScore >= 55) return "STRONG COLUMNSTORE CANDIDATE";
        if (t.CandidacyScore >= 35)
            return aggEvidence ? "GENUINE COLUMNSTORE CANDIDATE (workload evidence supports it)"
                               : "PROMISING CANDIDATE — test it";
        if (t.CandidacyScore >= 15 && insertOnly && t.PctMediumCardinalityColumns >= 50 && !tooSmall)
            return "MARGINAL — compression win possible, low priority";
        if (t.CandidacyScore >= 20) return "MARGINAL";
        return tooSmall ? "NOT A CANDIDATE (too small / wrong shape)" : "NOT A COLUMNSTORE CANDIDATE";
    }
    ...
    private static void WriteDataShape(StringBuilder sb, TableInfo t)
    {
        ...
        if (stars.Count > 0)
        {
            sb.Append("highly repetitive — " + string.Join(", ",
                stars.Select(c => $"`{c.ColumnName}` ({c.DistinctValues:N0} distinct, {c.DuplicationFactor:N0}x repeats)")));
        ...
        var jackpot = done.Where(c => c.IsStringType && c.AvgByteLength >= 20 && c.DuplicationFactor >= 50)
                          .OrderByDescending(c => c.AvgByteLength * (double)t.RowCount).FirstOrDefault();
        if (jackpot != null)
        {
            double mb = jackpot.AvgByteLength * t.RowCount / 1024.0 / 1024.0;
        ...
        double estMb = EstimateCompressedMb(t, done);
        if (estMb > 0 && t.BaseDataMb > 50 && estMb < (double)t.BaseDataMb * 0.6)
            sb.AppendLine($"Very rough clustered-columnstore size estimate: **~{estMb:N0} MB** vs {t.BaseDataMb:N0} MB rowstore data today.");
        sb.AppendLine();
    }
    ...
    private static void WriteWorkload(StringBuilder sb, TableInfo t, List<Finding> punch)
    {
        ...
        {
            parts.Add($"{t.UserSeeks:N0} seeks / {t.UserScans:N0} scans / {t.UserLookups:N0} lookups / {t.UserUpdates:N0} write ops since restart");
            if (insertOnly && t.LeafInserts > 0)
                ...
                parts.Add($"UPDATEs are {t.UpdateShareOfWritesPct:0}% of writes — the expensive operation on columnstore");
            ...
            if (t.PageIoLatchWaitMs > 10_000)
                parts.Add($"{t.PageIoLatchWaitMs / 1000:N0}s of IO-latch waits — IO-bound scans, which columnstore compression directly relieves");
        }
        ...
        if (topAgg != null && topAgg.TotalLogicalReads > 100_000)
        {
        ...
        else if (top != null && top.TotalLogicalReads > 500_000)
        {
            double perExec = top.ExecutionCount > 0 ? (double)top.TotalLogicalReads / top.ExecutionCount : 0;
            sb.AppendLine($"Heaviest cached query: {top.ExecutionCount:N0} executions, {top.TotalLogicalReads:N0} reads " +
    ...
    private static void WriteIndexFindings(StringBuilder sb, TableInfo t, List<Finding> punch)
    {
        ...
        bool isHeap = !t.Indexes.Any(i => i.TypeDesc.Contains("CLUSTERED", StringComparison.OrdinalIgnoreCase)
        ...
        {
            long reads = i.UserSeeks + i.UserScans + i.UserLookups;

            bool dead = (reads == 0 && i.UserUpdates > 0) ||
            ...
            var lobIncluded = SplitCols(i.IncludedColumns).Intersect(lobCols, StringComparer.OrdinalIgnoreCase).ToList();
            if (lobIncluded.Count > 0 && i.SizeMb >= t.BaseDataMb * 0.5m)
            {
                ...
                punch.Add(new Finding(1, $"{t.FullName}: rebuild `{i.IndexName}` without the LOB INCLUDE", (double)i.SizeMb * 0.9));
            }

            var firstKey = SplitCols(i.KeyColumns).FirstOrDefault();
            var keyCol = firstKey == null ? null
                       : t.Columns.FirstOrDefault(c => c.Analyzed && c.ColumnName.Equals(firstKey, StringComparison.OrdinalIgnoreCase));
            if (keyCol != null && keyCol.DistinctValues <= 10 && !dead && lobIncluded.Count == 0)
            ...
            var computedKeys = SplitCols(i.KeyColumns).Intersect(computedCols, StringComparer.OrdinalIgnoreCase).ToList();
            if (dead && computedKeys.Count > 0)
                findings.Add($"`{i.IndexName}` is keyed on computed column(s) {string.Join(", ", computedKeys.Select(c => $"`{c}`"))} " +
                             "— which exist only to feed this dead index");

            if (!dead && i.IncludedColumnCount >= 5)
                findings.Add($"`{i.IndexName}` ({i.SizeMb:N0} MB) is wide ({i.IncludedColumnCount} included columns) — " +
                             "the kind of covering index a columnstore replaces");
        }

        if (deadMb > 0)
            punch.Add(new Finding(1, $"{t.FullName}: drop the dead nonclustered index(es) — zero risk, immediate win", deadMb));
    ...
    private static void WriteRecommendation(StringBuilder sb, TableInfo t, List<Finding> punch)
    {
        ...
        var dateCol = t.Columns.FirstOrDefault(c => c.Analyzed &&
                        (c.DataType.StartsWith("date") || c.DataType == "datetime2" || c.DataType == "datetime"));
        ...
        if (t.CandidacyScore >= 35)
        {
            ...
            if (computedCols.Count > 0)
                steps.Add($"drop the computed column(s) {string.Join(", ", computedCols.Select(c => $"`{c}`"))} first " +
            ...
            if (t.WritePct >= 25 && !insertOnly)
                steps.Add("given the write mix, test a NONCLUSTERED columnstore first (keeps the rowstore PK, still gets batch mode)");
            ...
            punch.Add(new Finding(2, $"{t.FullName}: plan the columnstore conversion (test the heaviest query before/after)",
                Math.Max(0, (double)t.BaseDataMb - EstimateCompressedMb(t, t.Columns.Where(c => c.Analyzed).ToList()))));
        }
        else if (t.CandidacyScore >= 20 && insertOnly && !tooSmall)
        {
            sb.AppendLine("possible compression win but low absolute payoff — revisit after the higher-priority items.");
        }
        else
        {
    ...
    private static bool IsInsertOnly(TableInfo t) =>
        t.LeafInserts > 0 && t.LeafUpdates == 0 && t.LeafDeletes == 0;
    ...
    {
        double bytes = 0;
        foreach (var c in analyzed.Where(c => c.SampleRows > 0))
        {
            double factor = c.DuplicationFactor >= 1000 ? 0.02
                          : c.DuplicationFactor >= 100 ? 0.05
                          : c.DuplicationFactor >= 10 ? 0.15
                          : 0.5;
            bytes += c.AvgByteLength * t.RowCount * factor;
        }
