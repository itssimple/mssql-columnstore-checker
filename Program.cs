...
public static class Program
{
    ...
    private static void Run(AnalyzerOptions opt)
    {
        ...
        Console.WriteLine(
            $"SQL Server major version {major}, instance started {startTime:yyyy-MM-dd HH:mm} ({uptimeDays} days uptime)");
        if (uptimeDays < 7)
        ...
        foreach (var t in tables)
        {
            ...
            else
            {
                Console.Write("  - analyzing data cardinality" +
                              (t.RowCount > opt.SampleTargetRows * 2
                                  ? $" (sampling ~{opt.SampleTargetRows:N0} rows)"
                                  : " (full scan)") + " ... ");
                try
                {
        ...
        if (string.IsNullOrWhiteSpace(opt.LlmEndpoint)) return;
        ...
        {
            Console.WriteLine(
                "LLM narrative failed (non-fatal, rule-based analysis above still applies): " + ex.Message);
        }
    }
    ...
    private static AnalyzerOptions ParseArgs(string[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            {
                case "--server":
                case "-s": opt.Server = Next(); break;
                case "--database":
                case "-d": opt.Database = Next(); break;
                case "--user":
                case "-u": opt.User = Next(); break;
                case "--password":
                case "-p": opt.Password = Next(); break; // discouraged: lands in shell history
                ...
                case "--output":
                case "-o": opt.OutputFolder = Next(); break;
